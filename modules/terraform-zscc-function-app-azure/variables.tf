...
variable "function_app_dns_servers" {
  type        = list(string)
  description = "Optional custom DNS servers for the Function App. The first entry is set as WEBSITE_DNS_SERVER and the second (if present) as WEBSITE_DNS_ALT_SERVER. Required when resolving private endpoint records via a custom/hub DNS resolver. Defaults to [] (platform-default Azure DNS)."
  default     = []
}